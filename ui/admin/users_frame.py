...
class UsersFrame(ctk.CTkFrame):
    ...
    def _build_topbar(self):
        ...
        # ── Search entry ───────────────────────────────────────────────
        ctk.CTkLabel(self.topbar_frame, text="Search by userID or userusername", font=ctk.CTkFont(size=10)).grid(row=0, column=0, padx=(6,4), pady=(1,0), sticky="w")
        ...
        filter_frame.grid(row=1, column=2, padx=4, pady=0, sticky="nsew")
        ...
        for i, filter in enumerate(filter_list):
            ...
            chk.grid(row=row_idx, column=col_idx, padx=2, sticky=FORM_UI["entry_sticky"])
    ...
    def _build_table(self):
        ...
        # ── Table Section ───────────────────────────────────────────
        self.table = ttk.Treeview(self.table_frame, columns= ('user_id', 'username', 'password', 'role', 'status', 'date_created'), show= 'headings', selectmode="browse")
        ...
        self.table.heading('date_created', text= 'Date Created')

        # Column widths
        self.table.column('user_id',      width=150, minwidth=150, anchor='center')
        self.table.column('username',     width=250, minwidth=250)
        self.table.column('password',     width=150, minwidth=150)
        self.table.column('role',         width=150, minwidth=150, anchor='center')
        self.table.column('status',       width=200, minwidth=200)
        self.table.column('date_created', width=150, minwidth=150, anchor='center')
    ...
    def _refresh_table(self,rows):
        ...
        self.table.tag_configure('Active', background=DATA_FRAME_UI['user_active'])
        self.table.tag_configure('Inactive', background=DATA_FRAME_UI['user_inactive'])
        ...
        for row in rows:
            ...
            formatted_date = row['date_created'].strftime("%d-%m-%Y")
    ...
    def _on_row_select(self, event):
        column_usernames = ['user_id', 'username', 'password', 'role', 'status', 'date_created']
    ...
    def _open_form(self, mode):
        ...
        popup.title("Add user" if mode == "add" else "Edit user")
        ...
        self.date_created_var = ctk.StringVar()

        # Filling the date with Current date
        from datetime import datetime
        current_date = datetime.now().strftime("%d-%m-%Y")
        self.date_created_var.set(current_date)
        ...
        if mode == 'edit':
            self.username_var.set(self.selected_row['username'])
            self.pass_var.set(self.selected_row['password'])
            self.role_var.set(self.selected_row['role'])
            self.status_var.set(self.selected_row['status'])
            self.date_created_var.set(self.selected_row['date_created'])
    ...
    def _build_form_fields(self, popup):
        ...
        # Row 0 - username
        ctk.CTkLabel(form_frame, text="username:", font=ctk.CTkFont(family=DATA_FRAME_UI['btn_font_family'], size=DATA_FRAME_UI['btn_font_size'])).grid(row=0, column=0, padx=10, pady=(5,FORM_UI['row_pady']), sticky=FORM_UI["label_sticky"])
        ctk.CTkEntry(form_frame, textvariable=self.username_var).grid(row=0, column=1, padx=10, pady=FORM_UI['row_pady'], sticky=FORM_UI["entry_sticky"])

        # Row 1 - PAssword
        ctk.CTkLabel(form_frame, text="Password:", font=ctk.CTkFont(family=DATA_FRAME_UI['btn_font_family'], size=DATA_FRAME_UI['btn_font_size'])).grid(row=1, column=0, padx=10, pady=FORM_UI['row_pady'], sticky=FORM_UI["label_sticky"])
        ctk.CTkEntry(form_frame, textvariable=self.pass_var).grid(row=1, column=1, padx=10, pady=FORM_UI['row_pady'], sticky=FORM_UI["entry_sticky"])

        # Row 2 - role (Dropdown)
        ctk.CTkLabel(form_frame, text="role:", font=ctk.CTkFont(family=DATA_FRAME_UI['btn_font_family'], size=DATA_FRAME_UI['btn_font_size'])).grid(row=2, column=0, padx=10, pady=FORM_UI['row_pady'], sticky=FORM_UI["label_sticky"])
        ctk.CTkOptionMenu(form_frame, variable=self.role_var, values=["Select Role", "Admin", "Receptionist"], text_color='black', fg_color= DATA_FRAME_UI['btn_fg'], dropdown_hover_color= DATA_FRAME_UI['btn_hover']).grid(row=2, column=1, padx=10, pady=FORM_UI['row_pady'], sticky=FORM_UI["entry_sticky"])
        ...
        self.date_entry = ctk.CTkEntry(form_frame, textvariable=self.date_created_var)
        ...
        self.username_var.trace_add("write", self._validate)
        self.pass_var.trace_add("write", self._validate)
        self.role_var.trace_add("write", self._validate)
        self.status_var.trace_add("write", self._validate)
        self.date_created_var.trace_add("write", self._validate)
    ...
    def _validate(self, *args):
        ...
        # Required: username
        if not username:
            self._form_error("⚠ Username is required.")
            return
        if len(username) < 2 or len(username) > 50:
            self._form_error("⚠ Username must be 2-50 characters.")
            return
        
        # Required: Pass
        if len(password) < 8:
            self._form_error("⚠ Lenght of Password must be greater than 8.")
            return
        
        # Required: Role
        if role not in ('Admin', 'Receptionist'):
            self._form_error("⚠ Please select a role.")
            return
        ...
        if not date_created:
            self._form_error("⚠ Creation date is required.")
            return
        try:
            datetime.strptime(date_created, '%d-%m-%Y')   # tries to parse the date
        except ValueError:
    ...
    def _on_save(self, popup, mode):
        ...
        formatted_date = datetime.strptime(date_created, "%d-%m-%Y").strftime("%Y-%m-%d")

        if mode == 'add':
            success = insert_user(username, password, role, status, formatted_date)
        else:
            user_id = self.selected_row['user_id']
            formatted_id = user_id.replace("USR-","")
            success = update_user(formatted_id, username, password, role, status)
        ...
        if success:   # Insertion Successful
            ...
            messagebox.showerror(title="Error",
                                 message=f"Could not {mode} user. Check inputs or DB connection."
                                )

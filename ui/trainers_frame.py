...
class TrainersFrame(ctk.CTkFrame):
    ...
    def _build_topbar(self):
        ...
        filter_list = ["Active", "On-leave", "Terminated"]
        self.filters_var = {}
    ...
    def _on_search(self,  *args):
        ...
        if not search_term:
            rows = search_trainer(search_term, selected_filter)
            self._refresh_table(rows)
    ...
    def _build_form_fields(self, popup):
        ...
        ctk.CTkOptionMenu(form_frame, variable=self.status_var, values=["Select Status", 'Active', 'On-leave', 'Terminated'], text_color='black', fg_color= DATA_FRAME_UI['btn_fg'], dropdown_hover_color= DATA_FRAME_UI['btn_hover']).grid(row=4, column=1, padx=10, pady=FORM_UI['row_pady'], sticky=FORM_UI["entry_sticky"])
    ...
    def _on_save(self, popup, mode):
        ...
        if mode == 'add':
            ...
            formatted_id = trainer_id.replace("TRN-","")
            success = update_trainer(formatted_id, name, phone, salary, specs_string, status, defaultfee)
